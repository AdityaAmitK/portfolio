...
export function PostActions({ slug, published, previewUrl, compact = false }: { slug: string; published: boolean; previewUrl: string; compact?: boolean }) {
  ...
  const tracked = (source: string) => `${versionedPublicUrl()}&utm_source=${source}&utm_medium=social&utm_campaign=${encodeURIComponent(slug)}`
  ...
  return <div className={`post-actions${compact ? ' post-actions--compact' : ''}`}>
    ...
    <button className="admin-button admin-button--secondary" type="button" onClick={() => copyLink(published ? 'Public link' : 'Preview link', published ? versionedPublicUrl() : previewUrl)}>{copied || (published ? 'Copy public link' : 'Copy 24h preview')}</button>
    {published && <>
