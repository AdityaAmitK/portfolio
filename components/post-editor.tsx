...
export function PostEditor({ post, tags }: { post?: Post; tags: Tag[] }) {
  ...
  return <form action={upsertPost} className="admin-form">
    ...
    <div className="form-row"><fieldset className="field tag-picker"><legend>Tags</legend>{tags.length ? <div>{tags.map(tag => <label className="checkbox" key={tag.id}><input name="tags" value={tag.id} type="checkbox" defaultChecked={selectedTags.has(tag.id)} /> {tag.name}</label>)}</div> : <p className="muted">No existing tags yet.</p>}<div className="field new-tags"><label htmlFor="new-tags">Add new tags</label><input id="new-tags" name="newTags" placeholder="Backend, Architecture" /><small>Separate multiple new tags with commas. They will remain available for future articles.</small></div></fieldset><label className="checkbox"><input name="published" type="checkbox" defaultChecked={Boolean(post?.published)} /> Published</label></div>
