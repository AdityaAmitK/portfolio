...
const defaultSocialImage = { url: '/opengraph-image', width: 1200, height: 630, alt: 'Aditya Kinjawadekar — Software Engineer' }

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  const { slug } = await params
  const post = getPostBySlug(slug)
  if (!post) return {}
  const image = post.cover_image ? [{ url: socialImageFor(post.cover_image), width: 1200, height: 630, alt: post.cover_alt || post.title }] : [defaultSocialImage]
...
export default async function PostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const coverUrl = new URL(post.cover_image || defaultSocialImage.url, 'https://adityakinjawadekar.com').toString()
