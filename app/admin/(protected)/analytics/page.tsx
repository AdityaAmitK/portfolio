...
export default async function AnalyticsPage({ searchParams }: { searchParams: Promise<{ days?: string | string[]; path?: string | string[] }> }) {
  ...
  return <main className="admin-shell admin-main">
    ...
    <section className="analytics-section"><h2>Recent journeys</h2>{data.journeys.length ? <div className="analytics-journeys">{data.journeys.map(journey => <div key={journey.session_id}><div><strong>{journey.source}</strong><time>{new Date(`${journey.started_at}Z`).toLocaleString('en-IN', { dateStyle: 'medium', timeStyle: 'short' })}</time></div><p>{journey.pages}</p></div>)}</div> : <div className="empty-note">No journeys recorded in this period.</div>}</section>
